...
object App extends zio.ZIOAppDefault:
  ...
  private lazy val validateOrder: ValidateOrder =
    ...
    ValidateOrder(checkAddressExists, productCodeExists)

  private lazy val priceOrder: PriceOrder =
    PriceOrder(standardPrices, promoPrices)

  def placeOrder(unvalidatedOrder: UnvalidatedOrder): ZIO[Any, PlaceOrderErrorDto, PlaceOrderService.ValidatedOrder] =
    validateOrder(unvalidatedOrder).mapError(PlaceOrderErrorDto.fromDomain)
